...
public class AuthenticationService : AuthenticationStateProvider
{
    ...
    public override async Task<AuthenticationState> GetAuthenticationStateAsync()
    {
        ...
        ClaimsIdentity(JwtParser.ParseClaimsFromPayload(token),
    ...
    public async Task<AuthenticatedUserDTO> Login(AuthenticationUserModel userModel)
    {
        try
        {
            var user = new LoginUserDTO(userModel.Email, userModel.Password);
            ...
            HttpResponseMessage response = await _http.Client.PostAsync("tokens", jsonContent);
            ...
            {
                var tokenUser = new TokenUserDTO(userModel.Email);

                StringContent tokenJsonContent = new(
                    JsonSerializer.Serialize(tokenUser),
                    Encoding.UTF8,
                    "application/json");

                var createResponse = await _http.Client.PostAsync("tokens/create", tokenJsonContent);
            ...
            var result = JsonSerializer.Deserialize<AuthenticatedUserDTO>(responseContent, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
